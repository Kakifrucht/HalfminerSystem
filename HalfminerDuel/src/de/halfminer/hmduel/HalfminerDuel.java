                ...
                if (player.hasPermission("hmd.admin")) {
                    if (args.length == 1 && args[0].equalsIgnoreCase("reload")) {
                        ...
                        switch (args[0].toLowerCase()) {
                            case "create":
                                if (arenaManager.addArena(args[1], player.getLocation()))
                                    Util.sendMessage(player, "adminCreate", new String[]{"%ARENA%", args[1]});
                                else Util.sendMessage(player, "adminCreateFailed", new String[]{"%ARENA%", args[1]});
                                break;
                            case "remove":
                                if (arenaManager.delArena(args[1]))
                                    Util.sendMessage(player, "adminRemove", new String[]{"%ARENA%", args[1]});
                                else
                                    Util.sendMessage(player, "adminArenaDoesntExist", new String[]{"%ARENA%", args[1]});
                                break;
                            case "spawna":
                                if (arenaManager.setSpawn(args[1], player.getLocation(), true))
                                    Util.sendMessage(player, "adminSetSpawn", new String[]{"%ARENA%", args[1]});
                                else
                                    Util.sendMessage(player, "adminArenaDoesntExist", new String[]{"%ARENA%", args[1]});
                                break;
                            case "spawnb":
                                if (arenaManager.setSpawn(args[1], player.getLocation(), false))
                                    Util.sendMessage(player, "adminSetSpawn", new String[]{"%ARENA%", args[1]});
                                else
                                    Util.sendMessage(player, "adminArenaDoesntExist", new String[]{"%ARENA%", args[1]});
                                break;
                            case "setkit":
                                if (arenaManager.setKit(args[1], player.getInventory().getArmorContents(), player.getInventory().getContents()))
                                    Util.sendMessage(player, "adminSetKit", new String[]{"%ARENA%", args[1]});
                                else
                                    Util.sendMessage(player, "adminArenaDoesntExist", new String[]{"%ARENA%", args[1]});
                                break;
                            default:
