    ...
    public void onDismountGlitchCheck(VehicleExitEvent e) {
        ...
        for (int i = 0; i < 3; i++) {
            if (protectedMaterial.contains(w.getBlockAt(x, y + i, z).getType())) {
                if (hmc.isModuleEnabled(ModuleType.RESPAWN)) {
                    try {
                        ModRespawn modRespawn = (ModRespawn) hmc.getModule(ModuleType.RESPAWN);
                        scheduler.runTask(hmc, () -> {
                            modRespawn.tpToSpawn(p);
                            Message.create("modGlitchProtectionDismountTped", hmc, "AntiGlitch").send(p);
                            Message.create("modGlitchProtectionDismountTpedNotify", hmc, "AntiGlitch")
